  ...
  server.registerTool(
    'vicsee_generate',
    {
      ...
      inputSchema: {
        ...
        reference_image_urls: z.array(z.string()).optional().describe(`Reference-to-video only. Seedance 2.5 accepts up to 10 reference images; the 2.0 family (2.0 / Fast / Mini) up to 7. Each may be ${imageSourceHelp}. Refer to them in the prompt as @Image1, @Image2, …`),
        reference_video_urls: z.array(z.string()).optional().describe('Reference-to-video only: up to 3 public https video URLs, each 2s or longer. TOTAL across all clips must not exceed 30s on Seedance 2.5, or 15s on the 2.0 family (2.0 / Fast / Mini). The total is the binding limit, not the per-clip length.'),
        reference_audio_urls: z.array(z.string()).optional().describe('Reference-to-video only: up to 3 public https audio URLs. TOTAL across all clips must not exceed 30s on Seedance 2.5, or 15s on the 2.0 family. Accepted: mp3, wav, aac, m4a, ogg.'),
